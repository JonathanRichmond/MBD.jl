...
function rotating122Rotating41(dynamicsModel::BCR4BP12DynamicsModel, states12::Vector{Vector{Float64}}, times12::Vector{Float64})
    ...
    for t::Int16 = Int16(1):numTimes
        ...
        states41[t] = [(1/a4).*C zeros(Float64, 3, 4); sqrt(a4/(m4+1)).*Cdot sqrt(a4/(m4+1)).*C zeros(Float64, 3, 1); zeros(Float64, 1, 6) -1]*state+append!(1-get41MassRatio(dynamicsModel.systemData), zeros(Float64, 6))
    end
