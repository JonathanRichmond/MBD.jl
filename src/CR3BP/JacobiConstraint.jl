...
"""
function evaluateConstraint(jacobiConstraint::JacobiConstraint, freeVariableIndexMap::Dict{MBD.Variable, Int16}, freeVariableVector::Vector{Float64})
    return [getJacobiConstant(jacobiConstraint.dynamicsModel, getData(jacobiConstraint.state))-jacobiConstraint.value]
end

"""
    getNumConstraintRows(jacobiConstraint)
...
- `jacobiConstraint::JacobiConstraint`: Jacobi constraint object
"""
function getNumConstraintRows(jacobiConstraint::JacobiConstraint)
    return 1
end
...
- `freeVariableVector::Vector{Float64}`: Free variable vector
"""
function getPartials_ConstraintWRTVariables(jacobiConstraint::JacobiConstraint, freeVariableIndexMap::Dict{MBD.Variable, Int16}, freeVariableVector::Vector{Float64})
    numConstraints::Int16 = Int16(getNumConstraintRows(jacobiConstraint))
    mu::Float64 = getMassRatio(jacobiConstraint.dynamicsModel)
    q::StaticArrays.SVector{6, Float64} = StaticArrays.SVector{6, Float64}(getData(jacobiConstraint.state))
    r_13::Float64 = sqrt((q[1]+mu)^2+q[2]^2+q[3]^2)
    r_23::Float64 = sqrt((q[1]-1+mu)^2+q[2]^2+q[3]^2)
    r_13_3::Float64 = r_13^3
    r_23_3::Float64 = r_23^3
    partialWRTNodeState::Matrix{Float64} = zeros(Float64, (numConstraints,length(jacobiConstraint.state.data)))
    ...
    partials::Dict{MBD.Variable, Matrix{Float64}} = Dict{MBD.Variable, Matrix{Float64}}(jacobiConstraint.state => partialWRTNodeState)
