...
- `bounds::Matrix{Float64}`: Minimum/maximum values for each free variable
"""
function addBounds!(jumpCheck::BoundingBoxJumpCheck, problem::MBD.BCR4BP12MultipleShooterProblem, variable::MBD.Variable, bounds::Matrix{Float64})
    index0::Int16 = get(getFreeVariableIndexMap!(problem), variable) do
        err::String = "Variable $(variable.name) is not part of the problem"
        Logging.@error err
        throw(ArgumentError(err))
...
"""
function isFamilyMember(jumpCheck::BoundingBoxJumpCheck, data::MBD.BCR4BP12ContinuationData)
    freeVars::Vector{Float64} = getFreeVariableVector!(data.previousSolution)
...
- `variable::Variable`: Bounded free variable
"""
function removeBounds!(jumpCheck::BoundingBoxJumpCheck, problem::MBD.BCR4BP12MultipleShooterProblem, variable::MBD.Variable)
    index0::Int16 = get(getFreeVariableIndexMap!(problem), variable) do
        err::String = "Variable $(variable.name) is not part of the problem"
        Logging.@error err
        throw(ArgumentError(err))
